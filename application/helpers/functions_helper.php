 ...
 * Time: 12:17 PM
 */
function truncate_html($html, $max_length=500) {
    $dom = new DOMDocument();
    ...
    foreach ($nodes as $node) {
        ...
        if ($current_length + strlen($text) <= $max_length) {
            $result .= $text;
            $current_length += strlen($text);
        } else {
            $remaining_length = $max_length - $current_length;
            $result .= substr($text, 0, $remaining_length);
            break;
        }
    }
...
function getLimitText($string,$limit=500){
...
function makeNavbar($navbar_list){
    foreach ($navbar_list as $nav_element){
        $nav_element['NAVBAR_TITLE'] = ucwords($nav_element['NAVBAR_TITLE']);
        if(isset($nav_element['CHILDREN'])&&count($nav_element['CHILDREN'])){
            echo "<li ><a href='#'>{$nav_element['NAVBAR_TITLE']}</a>";
            echo "<ul class='dropdown'>";
            makeNavbar($nav_element['CHILDREN']);
            echo "</ul></li>";
        }else{
            $url = str_replace("<?=base_url()?>",base_url(),$nav_element['URL']);
            echo"<li ><a href='{$url}'>{$nav_element['NAVBAR_TITLE']}</a></li>";
        }
    }
}
function getChallanNoByFormFeeId($form_fee_id){
    $challan_no = SUTCP_CODE.str_pad($form_fee_id, 7, "0", STR_PAD_LEFT);
    return $challan_no;
}
...
function cryptPassowrd($password){
    $algorithm  = '$2a$07$youcantseethisisthesindhipoint$';
...
function sendPasswordTokenByEmail($email,$token,$user_id,$this_in=null){
...
// $headers .= 'Reply-To: director.itsc@usindh.edu.pk' . "\r\n";
    ...
    $res  = send_smtp_email($subject,$body,$email,$this_in);
...
function smsSender($to,$message){
	...
	$response = api_sms_zong($to,$message);
	return $response;
}
